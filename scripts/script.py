...
def analyze_dicom_files():
    ...
    results_file_path = os.path.join(RESULTS_DIR, f'results_{timestamp}.json')
...
def create_dicom_sr_for_files():
    """Cria arquivos DICOM SR para imagens DICOM existentes no diretório DICOM e os salva com resultados específicos da análise."""
        ...
        for file in files:
            if file.endswith(".dcm"):
                ...
                try:
                    ...
                    # Salva o dataset SR apenas no diretório SR_DIR
                    sr_file_path_sr_dir = os.path.join(SR_DIR, os.path.basename(dicom_file_path).replace('.dcm', '_sr.dcm'))
                    sr_ds.save_as(sr_file_path_sr_dir)
                    print(f"Arquivo DICOM SR salvo no diretório SR_DIR: {sr_file_path_sr_dir}")

                except Exception as e:
                    print(f"Falha ao criar DICOM SR para {dicom_file_path}: {e}")


def upload_dicom_sr_files():
    """Faz o upload dos arquivos DICOM SR gerados no SR_DIR para o servidor Orthanc e os move para um diretório de arquivo."""

    # Verifica se o diretório SR_DIR existe
    if not os.path.isdir(SR_DIR):
        print(f"O diretório {SR_DIR} não existe.")
        return

    sr_files = []
    archive_dir = os.path.join(SR_DIR, 'old_sr')

    # Cria um diretório de arquivo dentro da pasta SR_DIR se não existir
    if not os.path.exists(archive_dir):
        os.makedirs(archive_dir)

    # Percorre todos os subdiretórios em SR_DIR para encontrar arquivos SR, excluindo o diretório 'old_sr'
    for root, dirs, files in os.walk(SR_DIR):
        if os.path.abspath(root) == os.path.abspath(archive_dir):
            continue
        
        for file in files:
            if file.endswith("_sr.dcm"):  # Garante que seja um arquivo DICOM SR
                file_path = os.path.join(root, file)
                sr_files.append(file_path)

    # Verifica se algum arquivo DICOM SR foi encontrado
    if not sr_files:
        print(f"O diretório {SR_DIR} não contém arquivos DICOM SR para upload.")
        return

    # Faz o upload de cada arquivo DICOM SR
    for file_path in sr_files:
        ...
        with open(file_path, 'rb') as file_data:
            ...
            if response.status_code == 200:
                print(f"Upload bem-sucedido de {os.path.basename(file_path)}")
                # Move o arquivo para o diretório 'old_sr' após o upload bem-sucedido
