...
### 4) EXTRA: Criar um DICOM SR (Structured Report) para cada arquivo DICOM com os resultados do modelo, e enviar os DICOM SR para o PACS local OrthanC. 
Extensivamente a etapa mais díficil, onde diversos problemas foram encontrados durante toda a etapa, principalmente relacionados à parâmetros não preenchidos necessários como 'SOPClassUID', 'Dataset.is_little_endian', 'Dataset.is_implicit_VR', etc. De longe o maior problema foi a aparição de '*instance:null*' após conseguir fazer o upload dos *SR*s, correlacionada à uma criação errada dos SRs. Para identificar os parâmetros errados ou não preenchidos, utilizei de bibliotecas e toolkits como *dicom-validator* ou *dcmtk* para verificar as *tags* dos *SR*s criados.

Posteriormente, acabei entrando em contato com o próprio desenvolvedor do sistema OrthanC (Sebastién Jodogne), além de contato com outros desenvolvedores pelo [fórum OrthanC](https://discourse.orthanc-server.org/), porém mesmo assim não consegui solucionar o problema. Apesar de conseguir enviar os *SR*s para o servidor local, e estarem com informações corretas de análise, ainda assim nem todas as funcionalidades eram permitidas com as instâncias. A última etapa de validação era comparar com outros *SR*s que eram válidos, porém não foi possível encontrar *samples* desses arquivos de fácil acesso para fazer tal comparação. Portanto, o desafio não foi completamente realizado, faltando permitir todas as funcionalidades possível com arquivos *SR*s no servidor local (como *preview*).
